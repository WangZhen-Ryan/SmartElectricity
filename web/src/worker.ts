...
type BacktestPoint = {
  ...
  signalAction?: "charge" | "discharge" | "hold";
  executedAction?: "charge" | "discharge" | "hold";
};
...
function runBacktest(
  ...
  let soc = normalizeStartSocKwh(config.startSoc, config.capacityKwh);
  let cash = 0;
  ...
  return market.map((m, index) => {
    ...
    const signalAction: BacktestPoint["signalAction"] = decision.sell
      ? "discharge"
      : decision.buy
        ? "charge"
        : "hold";
    let executedAction: BacktestPoint["executedAction"] = "hold";
    let blockedReason: BacktestPoint["blockedReason"] = null;
    ...
    if (decision.sell) {
      ...
      if (discharge > 0) {
        ...
        executedAction = "discharge";
      } else {
        blockedReason = soc <= 0 ? "soc-low" : "power-limit";
      }
      ...
      if (charge > 0) {
        ...
        executedAction = "charge";
      } else {
        blockedReason = config.capacityKwh - soc <= 0 ? "soc-high" : "power-limit";
      }
    }
...
function normalizeStartSocKwh(startSoc: number, capacityKwh: number) {
  const cap = Math.max(0, capacityKwh);
  if (cap === 0) return 0;
  // Backward compatibility: many sessions set startSoc as percentage (0-100).
  if (startSoc > cap && startSoc <= 100) {
    return (startSoc / 100) * cap;
  }
  return Math.max(0, Math.min(startSoc, cap));
}
